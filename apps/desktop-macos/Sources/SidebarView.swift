...
struct SidebarView: View {
        ...
        if runtime.workspaceGroups.isEmpty && runtime.workspaces.isEmpty {
            ...
            if !ungrouped.isEmpty {
                ...
                ForEach(ungrouped) { workspace in
                    workspaceSection(workspace, indent: 0)
                }
            }
        }
    }
        ...
        if expanded {
            if groupWorkspaces.isEmpty {
                ...
                ForEach(groupWorkspaces) { workspace in
                    workspaceSection(workspace, indent: 14)
                }
            }
        }
    }
    ...
    @ViewBuilder
    private func workspaceSection(_ workspace: Workspace, indent: CGFloat) -> some View {
        ...
        WorkspaceRow(
            ...
            actions: WorkspaceRowActions(
                ...
                moveToGroup: { groupId in
                    let group = runtime.workspaceGroups.first { $0.id == groupId }
                    Task { await model.moveWorkspace(workspace, to: group) }
                },
