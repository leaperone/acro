...
11. Agent 需要用户输入时，移动端可以定位到对应会话；用户必须进入真实终端完成回答，OSC 状态不能生成任何可执行输入。未知 Agent 仍安全降级为普通终端。
12. Runtime 保存可恢复的通知与未读状态。iOS 后台推送只携带客户端生成的随机路由标识和不透明通知 ID，详细内容在客户端连接对应 Runtime 后通过 E2EE 获取。
...
- Agent 状态：供应商外置适配器、通用状态、会话定位和等待输入提醒。
...
- 不从 OSC 消息生成审批按钮、候选回答、终端输入或其他可执行动作。
- 不把敏感问题正文、仓库路径、终端内容、Acro 配对认证 token 或 E2EE 密钥发送给推送服务。Expo push token 是第三方寻址所必需，必须按凭证管理并从日志脱敏。
...
- Agent 状态只能来自显式适配器生成的受限消息。消息只表达 idle、working、needs_user、done、error，不携带问题正文、候选回答或可执行输入，也不授予文件、Git、Shell 或系统权限。
- Agent 状态属于同一 PTY 内可伪造的非可信提示。daemon 必须按 Session 去重相同状态、合并高频变化；needs_user 通知和 Push 必须有界，超限时只保留 Runtime 当前事实与未读记录。
- 通知记录由 Runtime 持有。设备撤销后必须删除其推送 token，并停止向该设备发送通知。
- 每个配对 Server 的客户端必须生成独立随机 pushRouteId；Push 只能携带 pushRouteId 与不透明 notificationId，客户端只按本地映射选择 Runtime。
...
- AgentActivity：sessionId、state、updatedAt。
- state：idle、working、needs_user、done、error。

Agent 适配器通过受限 OSC 消息把状态写入当前 PTY。terminal daemon 只解析带 Acro namespace、版本号、长度上限和 schema 的显式状态消息；普通终端输出保持不透明。daemon 把最新状态绑定到 sessionId 并纳入会话快照，对相同状态去重并把高频变化合并为最新事实。Runtime 负责镜像和广播 agent.activityChanged。

needs_user 只产生通知和会话定位入口。用户点击后进入真实终端，并按现有 session.claimFocus 规则取得输入权后自行回答。客户端不得从 OSC 生成按钮、文本或 terminal input。Acro 不调用供应商私有审批 API。没有适配器的 Agent 只显示终端，不产生虚假状态。
...
- device.setPushToken 只允许当前设备登记或清除自己的 token 与 pushRouteId。pushRouteId 由客户端为每个配对 Server 独立随机生成，不包含 serverId、地址或用户信息。

前台连接直接收 E2EE 事件。iOS 后台可选用 Expo Push API 发送固定标题、pushRouteId 和 opaque notificationId；App 用 pushRouteId 查询本地 ServerEntry，再连接对应 Runtime 读取真实内容。未知 route 必须安全忽略；删除配对或轮换 route 时同步清理 Runtime 登记与本地映射。没有推送 token 或推送服务不可达时，通知仍保存在 Runtime，前台恢复后可见。
...
| Agent 状态 | packages/protocol、apps/runtime/src/daemon、apps/runtime/src/index.ts、Desktop/Mobile 状态 UI | 受限 OSC 适配器、通用状态、等待输入提醒与会话定位 |
| 通知 | packages/protocol、apps/runtime/src/devices.ts、通知存储模块、apps/mobile | 持久通知、按设备未读、push token、pushRouteId、后台唤醒 |
...
5. 新增 AgentActivity、NotificationRecord、通知 RPC 和 Agent/通知事件 schema。
6. 生成 Swift 类型并更新 TypeScript 导出。

验证：

- Protocol schema、Agent/通知模型、frame round-trip、短帧、未知 codec、非法尺寸和序号测试。
- Swift codegen 无手工差异。
- 旧请求仍可解析，旧帧仍可解码。
...
### 阶段 7A：通用 Agent 状态与会话定位

入口条件：

- 阶段 0 已冻结 AgentActivity 和对应事件 schema。
...
2. terminal daemon 保存每个存活 Session 的最新 AgentActivity，对相同状态去重，并在高频切换时只发布最新事实；Runtime 启动或重连 daemon 后重新拉取快照并广播，Session 删除时一并清理。
...
5. needs_user 只触发通知和定位，不生成原生审批动作；用户进入真实终端后按现有 session.claimFocus 与显式接管规则输入。
6. 适配器不可用、事件未知或 schema 不匹配时安全忽略，不把终端标题或普通文本猜成状态，也不产生 terminal input。
...
- 多设备同时查看、会话定位和显式接管测试。
- 伪造 OSC、恶意状态载荷和重复 needs_user 都不能生成终端输入或可执行客户端动作。
- 伪造进程交替发送状态时，状态广播、通知记录和后台 Push 次数保持有界，最终状态仍可恢复。
- Runtime 重启后仍能从 Session 当前事实恢复，不把旧状态误报为当前等待输入。
...
2. 新增 notification.list、notification.markRead、notification.unreadCount 和 device.setPushToken；push token 登记同时保存当前配对 Server 的 pushRouteId。
3. 前台客户端通过 E2EE 事件即时更新未读；断线重连后通过游标补齐，不依赖事件恰好在线送达。同一 Session 的重复 needs_user 去重，高频状态变化只更新 Runtime 记录；每个设备与 Session 的后台 Push 使用有界限频器。
4. 移动端接入 expo-notifications。后台推送只发送固定文案、pushRouteId 和 opaque notificationId，不发送 serverId、地址、仓库、问题或终端内容。
5. App 用 pushRouteId 映射本地 ServerEntry，连接对应 Runtime，读取真实记录并定位 Workspace 与 Session；未知 route 不尝试跨服务器猜测。
6. 删除配对、route 轮换、设备撤销、Expo push token 轮换、推送失败和通知保留上限都由 Runtime 与客户端同步处理；Expo push token 和可选 Expo access token 按凭证存储并从日志脱敏，推送失败不丢通知。

验证：

- 通知持久化、去重、分页、按设备已读、撤销设备和保留上限测试。
- App 前台、后台、离线、推送 token 失效、多服务器相同 notificationId、未知 route 和 route 轮换测试。
- 重复和交替 needs_user 状态下，通知记录与每设备/Session Push 次数保持有界，未推送事件仍能从 Runtime 补齐。
- 抓包确认推送服务只收到必需的 Expo push token、可选 Expo access token、pushRouteId 和 notificationId，收不到 serverId、地址、Acro 配对认证 token、E2EE 密钥、敏感正文、路径或终端内容。
...
| Agent 状态 | daemon OSC parser、适配器夹具、状态、会话定位、伪造消息无输入副作用、多设备接管与降级测试 |
| 通知 | Runtime 持久化、按设备未读、pushRouteId 路由、推送隐私、离线补齐和多服务器冲突测试 |
...
| OSC 不生成任何可执行输入 | 同一 PTY 内进程可伪造 OSC；needs_user 只负责通知和定位，回答留在真实终端 |
| Runtime 持有通知和按设备未读 | iOS 后台可能断开，在线事件不能证明通知可靠送达 |
| Push 使用随机 pushRouteId 和 opaque notificationId | 客户端可在多服务器间可靠选中 Runtime，推送服务仍看不到服务器身份和敏感正文 |
