...
- 不预建空应用和空共享包。apps/cli 只在第一个可运行命令落地时创建。
...
- 每项能力至少包含 available 和稳定 reasonCode；浏览器另报 stream 与 automation，Simulator 另报 stream 与 input，Computer Use 另报 providerProtocolVersion。
...
- 同一设备重连可以恢复自己的控制权。
- 设备所有连接都断开后释放控制权。
- 关闭 Surface 前在同一领域串行器内校验控制权和存活状态。
...
| CLI 与 skills | 实现首个命令时创建 apps/cli；skills 与 CLI 源码同目录 | browser、emulator、computer 命令和版本一致的说明 |
...
5. 终端专属操作只对 terminal Surface 生效。普通切换、分屏和关闭操作对四种 Surface 共用。

验证：

- 旧布局迁移、混合 Surface 布局、拖拽、分屏、关闭、重连和失效引用测试。
...
6. UI 继续通过 Acro E2EE 画面与输入通道控制浏览器，不连接 CDP。

验证：

- 多页面目标隔离、lease 过期、CLI 中断、浏览器关闭和 Runtime 断线测试。
- 浏览器画面尺寸、点击映射、键盘、滚轮、慢客户端丢帧测试。
...
1. 创建最小 apps/cli，复用 packages/protocol 和 ~/.acro/client.json。
...
6. CLI 通过本机受限 socket 获取当前服务与 Surface 上下文；缺少或歧义时给出可操作错误。
