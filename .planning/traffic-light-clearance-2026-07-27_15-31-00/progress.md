...
- 每个 WorkbenchView 独立保存全屏状态，并通过 Bonsplit 渲染 override 只影响所属窗口。
...
| 初版 `fullScreenNotificationsUpdateTrafficLightClearance` | enter fullscreen 后渲染仍保留 `80pt` | expected fail |
| `fullScreenNotificationsStayScopedToTheirWindow` | 两个窗口的 enter/exit 通知互不污染 | pass |
| `tabBarLeadingInsetOverrideDoesNotMutateTheSharedController` | 当前渲染树可显示 0pt，controller 仍保持 80pt | pass |
| `TerminalPanesInteractionTests` | 47 个交互测试全部通过 | pass |
| `swift test --package-path apps/desktop-macos` | XCTest 86 个 + Swift Testing 60 个全部通过 | pass |
| `swift test --package-path apps/desktop-macos/Vendor/bonsplit` | 204 个 Bonsplit 测试全部通过 | pass |
...
| 直接给普通 NSWindow 插入 `.fullScreen` 做合成测试会触发 AppKit 异常 | 1 | 不伪造非法 styleMask；用权威通知和渲染 override 行为测试覆盖 |
| 尝试先提交失败测试时 development guard 拒绝未完成 planning | 1 | 不绕过 guard；先完成实现与验证，再按 staged 边界生成“红测 / 修复”两个提交 |
| preflight 首次 merge-tree 使用了旧三参数形式 | 1 | 改用当前 Git 的 `--merge-base` 形式；确认与 origin/main 无冲突 |
| preflight 发现共享 model fullscreen 状态会污染多窗口 | 1 | 移除共享 bool，改为每个 WorkbenchView 的渲染局部 override，并增加窗口隔离测试 |
| 首版 override 给 internal View 增加必填参数，Bonsplit 测试目标无法编译 | 1 | internal 参数改为默认 nil，完整 204 项 Bonsplit 测试通过 |
