...
- 用 refresh job 序号绑定删除确认，只允许删除成功后启动的缺失快照清 tombstone。
...
| 最终 `swift test` | 87 XCTest + 56 Swift Testing，共 143 项通过 | 通过 |
...
| 最新 `swift test --filter TerminalPanesInteractionTests` | 43 项通过 | 通过 |
...
| 删除成功前启动的旧 refresh 可能提前清 tombstone | 1 | 记录删除成功时的 refresh job floor，只接受更晚启动的缺失快照 |
